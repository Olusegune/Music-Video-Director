...
export function buildZip(entries: { name: string; bytes: Uint8Array }[]): Blob {
  ...
  const localSize = files.reduce(
    (total, file) => total + 30 + file.nameBytes.length + file.bytes.length,
    0
  );
  ...
  for (const file of files) {
    ...
    u16(view, offset + 8, 0);
    u32(view, offset + 14, file.crc);
    u32(view, offset + 18, file.bytes.length);
    u32(view, offset + 22, file.bytes.length);
    u16(view, offset + 26, file.nameBytes.length);
  ...
  files.forEach((file, index) => {
    ...
    u32(view, offset + 16, file.crc);
    u32(view, offset + 20, file.bytes.length);
    u32(view, offset + 24, file.bytes.length);
    u16(view, offset + 28, file.nameBytes.length);
  ...
  u16(view, offset + 8, files.length);
  u16(view, offset + 10, files.length);
  u32(view, offset + 12, offset - centralOffset);
