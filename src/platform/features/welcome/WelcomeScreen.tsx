    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-[#07080b] p-4 sm:p-6">
      <div className="relative flex h-full max-h-[880px] w-full max-w-6xl overflow-hidden rounded-2xl border border-white/10 bg-[#101216] shadow-2xl shadow-black/70">
        <section className="relative hidden w-[46%] overflow-hidden lg:block">
          <img
            src={heroArt}
            alt="Afrofuturistic creator directing a connected world of film, music, design, fashion, products, and storytelling"
          ...
          <div className="absolute inset-x-0 bottom-0 p-8">
            ...
            <div className="mt-5 flex flex-wrap gap-2 text-[11px] text-white/65">
              {[Library, Users, Map, Package].map((Icon, i) => (
                <span
                  key={i}
                  className="flex items-center gap-1.5 rounded-full border border-white/10 bg-black/30 px-2.5 py-1.5"
                >
                  <Icon className="h-3 w-3" />
                  {["Assets", "Characters", "Locations", "Props"][i]}
                </span>
              ))}
            </div>
          </div>
        ...
        <section className="flex min-w-0 flex-1 flex-col overflow-y-auto p-6 sm:p-8 lg:p-10">
          <button
            ...
            className="absolute right-4 top-4 z-10 rounded-lg border border-white/10 bg-black/25 p-2 text-white/60 transition hover:bg-white/10 hover:text-white"
          >
            <X className="h-4 w-4" />
          </button>
          <div className="max-w-2xl pr-8">
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-primary/25 bg-primary/10 px-3 py-1.5 text-xs font-semibold text-primary">
              <Sparkles className="h-3.5 w-3.5" /> AI creative operating system
            </div>
            <h1 className="text-3xl font-black tracking-tight text-white sm:text-4xl">
              Welcome to Director Studio
            </h1>
            <p className="mt-3 max-w-xl text-sm leading-relaxed text-white/60 sm:text-base">
              Move from an idea to a connected campaign, film, experience, or visual world—without
              rebuilding context in every tool.
            </p>
          </div>
          ...
          <div className="mt-7 rounded-xl border border-primary/25 bg-gradient-to-br from-primary/15 via-white/[0.03] to-transparent p-5">
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
              <div>
                <p className="font-semibold text-white">What do you want to create?</p>
                <p className="mt-1 text-xs text-white/50">
                  Director will help choose the workflow and keep every deliverable connected.
                </p>
              </div>
              <Button size="lg" onClick={() => close(() => setWizardOpen(true))}>
                Start with Director <ArrowRight className="h-4 w-4" />
              </Button>
            </div>
          </div>
          ...
          <div className="mt-7 grid gap-2.5 sm:grid-cols-2">
            {modules.map(({ key, title, desc, icon: Icon, accent }, index) => (
              <button
                ...
                className={`group flex items-center gap-3 rounded-xl border border-white/10 bg-gradient-to-br ${accent} p-3.5 text-left transition duration-200 hover:-translate-y-0.5 hover:border-white/25 ${index === 4 ? "sm:col-span-2" : ""}`}
              >
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-black/25 text-white/85">
                  <Icon className="h-5 w-5" />
                </span>
                <span className="min-w-0 flex-1">
                  <span className="block text-sm font-semibold text-white">{title}</span>
                  <span className="mt-0.5 block text-xs text-white/50">{desc}</span>
                </span>
          ...
          <div className="mt-auto flex flex-wrap items-center justify-between gap-3 border-t border-white/10 pt-5 text-xs">
            ...
            >
              <LifeBuoy className="h-3.5 w-3.5" /> Help & learning
            </button>
          </div>
        </section>
      </div>
