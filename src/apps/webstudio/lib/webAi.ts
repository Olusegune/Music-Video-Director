...
export function parsePositioning(raw: string): Positioning {
  ...
  if (
    ...
    !strings(value.valueProps) ||
    !strings(value.objections) ||
    !strings(value.proof)
  ) {
    throw new Error("The positioning response did not match the required schema.");
  }
  if (value.valueProps.length < 3 || value.objections.length < 2 || value.proof.length < 1)
    throw new Error("The positioning response was incomplete.");
  return {
    audience: value.audience,
    offer: value.offer,
    promise: value.promise,
    cta: value.cta,
    valueProps: value.valueProps.slice(0, 5),
    objections: value.objections.slice(0, 5),
    proof: value.proof.slice(0, 6),
  };
}

export function parseCopyDrafts(
  raw: string,
  allowedPatternIds: string[]
): Record<string, SectionCopy> {
  const value = JSON.parse(raw) as { sections?: unknown };
  if (!Array.isArray(value.sections))
    throw new Error("The copy response did not contain a sections array.");
  ...
  for (const entry of value.sections) {
    ...
    if (
      ...
      throw new Error("A section copy entry did not match the required schema.");
    output[item.patternId] = {
  ...
  if (Object.keys(output).length !== allowed.size)
    throw new Error("The copy response omitted one or more selected sections.");
  return output;
}
