...
import { VIDEO_MODELS, videoCaps } from "@/platform/lib/videoGen";
...
export function ShotRow({
  ...
  const imgModelLabel = GEN_MODELS.find((m) => m.id === effectiveImageModel)?.label ?? "Frame";
  ...
  const toggleEmotion = (emotion: string) => {
    ...
    const next = re.test(cur)
      ? cur
          .replace(new RegExp(`\\s*,?\\s*\\b${emotion}\\b\\s*,?`, "i"), " ")
          .replace(/\s{2,}/g, " ")
          .replace(/^[,\s]+|[,\s]+$/g, "")
      : cur
        ? `${cur}, ${emotion}`
    ...
    >
      ...
      <div className="flex items-center gap-2 border-b border-border/60 bg-elevated/30 px-3 py-2">
        ...
        <span className="ml-auto flex items-center gap-2 text-[10px] tabular-nums text-muted">
          ...
          {bpm > 0 && (
            <span className="hidden sm:inline">{beatRangeLabel(shot.start, shot.end, bpm)}</span>
          )}
          <span
      ...
      <div className="flex flex-wrap gap-3 p-3">
        <div className="min-w-[280px] flex-1 space-y-2.5">
          {shot.lyric ? (
            ...
            <p className="text-[10px] italic text-muted/60">
              No lyric in this window — instrumental / atmosphere.
            </p>
          )}
          <MentionTextarea
            value={shot.idea}
          ...
          {(continuity.prevMatches || continuity.firstAppearance || continuity.energyRising) && (
            <div className="rounded-md border border-warning/30 bg-warning/5 px-2 py-1.5">
              <p className="text-[11px] font-semibold uppercase tracking-wide text-warning/80">
                🔗 Continuity
              </p>
              ...
              {shot.promptOverride ? (
                <>
                  <textarea
                    ...
                    onChange={(e) =>
                      onChange({ ...shot, promptOverride: e.target.value || undefined })
                    }
                    rows={6}
              ...
              <div className="flex flex-wrap gap-1.5">
                {caps.endFrame && (
                  <OmniSlot
                    ...
                    onPick={async (f) => onChange({ ...shot, endFrame: await fileToDataUrl(f) })}
      ...
      {preview && (
        <ShotPreview kind={preview} shot={shot} index={index} onClose={() => setPreview(null)} />
      )}
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-[60] flex flex-col bg-black/90" onClick={onClose}>
      ...
      >
        {kind === "image" && (
          <>
            <Button
              variant="secondary"
              size="sm"
              onClick={() => setZoom((z) => Math.max(0.25, (fit ? 1 : z) - 0.25))}
              aria-label="Zoom out"
            >
            ...
            <Button
              variant="secondary"
              size="sm"
              onClick={() => setZoom((z) => Math.min(5, (fit ? 1 : z) + 0.25))}
              aria-label="Zoom in"
            >
        ...
        <Button
          ...
          size="sm"
          onClick={() =>
            downloadAsset(src, `shot-${index + 1}.${kind === "image" ? "png" : "mp4"}`)
          }
        >
