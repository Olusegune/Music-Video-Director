...
export function buildStrategyPdf(campaign: CampaignProject): Uint8Array {
  const lines = [campaign.name, "", "STRATEGY", campaign.strategy.positioning, campaign.strategy.keyMessage, "", "MESSAGE PILLARS", ...campaign.strategy.pillars, "", "AUDIENCE INSIGHT", campaign.strategy.audienceInsight, "", "CAMPAIGN CONCEPT", campaign.concept.bigIdea, campaign.concept.tagline, campaign.concept.visualWorld];
  const wrapped = lines.flatMap((line) => line.match(/.{1,82}(?:\s|$)/g)?.map((part) => part.trim()) ?? [line]).slice(0, 42);
  const stream = ["BT", "/F1 12 Tf", "72 750 Td", ...wrapped.flatMap((line, index) => index === 0 ? [`(${pdfEscape(line)}) Tj`] : ["0 -16 Td", `(${pdfEscape(line)}) Tj`]), "ET"].join("\n");
  const objects = [
...
export function buildPlanCsv(campaign: CampaignProject) {
  return ["title,channel,owner,status,due_offset_days,brief", ...campaign.plan.map((item) => [csv(item.title), csv(item.channel), csv(item.ownerModule), csv(item.content ? "approved" : "planned"), csv(item.dueOffset), csv(item.brief)].join(","))].join("\n");
}
