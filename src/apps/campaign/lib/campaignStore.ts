...
export function listCampaigns(): CampaignProject[] {
  try {
    return JSON.parse(localStorage.getItem(LS) ?? "[]") as CampaignProject[];
  } catch {
    return [];
  }
}
export function saveCampaign(project: CampaignProject): CampaignProject {
...
export function deleteCampaign(id: string) {
  localStorage.setItem(LS, JSON.stringify(listCampaigns().filter((project) => project.id !== id)));
}
