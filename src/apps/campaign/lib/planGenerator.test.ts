...
import {
  ...
  planBlueprint,
  produceNativeCopy,
} from "@/apps/campaign/lib/planGenerator";
...
describe("Campaign plan generator", () => {
  ...
  it("creates linked deliverables and strategy-rich briefs", () => {
    ...
    expect(plan.every((item) => item.brief.includes(strategy.keyMessage))).toBe(
      true
    );
    expect(
      JSON.parse(localStorage.getItem("mf.deliverables") ?? "[]")
    ).toHaveLength(8);
  });
